...
def lambda_handler(event, context):
    try:
        ...
        if not resume_data:
            print("No item found in DynamoDB for the given key")
            return {
                'statusCode': 404,
                'body': json.dumps({'error': 'Item not found'})
            }
        
        print("Fetched item from DynamoDB:", resume_data)

        ordered_resume_data = OrderedDict([
            ("id", resume_data.get("id", {}).get("S")),
            ("name", OrderedDict([
                ("Full Name", resume_data.get("name", {}).get("M", {}).get("Full Name", {}).get("S"))
            ])),
            ("basics", OrderedDict([
        ...
        print("Ordered resume data:", ordered_resume_data)
